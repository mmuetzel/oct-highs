...
## -*- texinfo -*-
## @deftypefn {} {[@var{xopt}, @var{fmin}, @var{errnum}, @var{extra}] =} highs (@var{c}, @var{A}, @var{b}, @var{lb}, @var{ub}, @var{ctype}, @var{vartype}, @var{sense}, @var{param})
## Solve a linear or mixed-integer programming problem using the HiGHS @sc{highs} library.
##
## Given three arguments, @code{highs} solves the following standard LP:
## @tex
## $$
##   \min_x C^T x
## $$
...
##    highs (c, A, b, lb, ub, ctype, vartype, s, param);
## @end group
## @end example
## @end deftypefn

function [xopt, fmin, errnum, extra] = highs (c, A, b, lb, ub, ctype, vartype, sense, param)
  ...
  if (nargin < 3)
   ...
   if (! isvector (c) || iscomplex (c) || ischar (c) || any (! isfinite (c)))
     error ("highs: C must be a real vector with finite values");
  endif
  nx = length (c);
  ## Force column vector.
  c = c(:);
  ...
  if (isempty (A))
    error ("highs: A cannot be an empty matrix");
  endif
  if (! isreal (A))
    error ("highs: A must be real valued, not %s", typeinfo (A));
  endif
  if (any (! isfinite (A(:))))
    error ("highs: The values in A must be finite");
  endif

  [nc, nxa] = size (A);
  if (nxa != nx)
    error ("highs: A must be %d-by-%d, not %d-by-%d",
  ...
  if (isempty (b))
    error ("highs: B cannot be an empty vector");
  endif
  if (! isreal (b) || length (b) != nc)
    error ("highs: B must be a real-valued %d-by-1 vector", nc);
  endif
  if (any (! isfinite (b(:))))
    error ("highs: The values in B must be finite");
  endif
  ...
  if (nargin > 3)
    ...
    elseif (! isreal (lb) || all (size (lb) > 1) || length (lb) != nx
            || any (isnan (lb)))
      error ("highs: LB must be a real-valued %d-by-1 column vector", nx);
    endif
  else
    lb = zeros (nx, 1);
  endif
  ...
  if (nargin > 4)
    ...
    elseif (! isreal (ub) || all (size (ub) > 1) || length (ub) != nx
            || any (isnan (ub)))
      error ("highs: UB must be a real-valued %d-by-1 column vector", nx);
    endif
  else
    ub = Inf (nx, 1);
  endif
  ...
  if (nargin > 5)
    ...
    elseif (! ischar (ctype) || all (size (ctype) > 1) || length (ctype) != nc)
      error ("highs: CTYPE must be a char vector of length %d", nc);
    elseif (! all (ctype == "F" | ctype == "U" | ctype == "S"
                   | ctype == "L" | ctype == "D"))
      error ("highs: CTYPE must contain only F, U, S, L, or D");
    endif
  else
    ctype = repmat ("S", nc, 1);
  endif
  ...
  if (nargin > 6)
    ...
    elseif (! ischar (vartype) || all (size (vartype) > 1)
            || length (vartype) != nx)
      error ("highs: VARTYPE must be a char vector of length %d", nx);
    elseif (! all (vartype == "C" | vartype == "I"))
      error ("highs: VARTYPE must contain only C or I");
    endif
  else
  ...
  if (nargin > 7)
    ...
    elseif (ischar (sense) || all (size (sense) > 1) || ! isreal (sense)
            || any (! isfinite (sense)))
      error ("highs: SENSE must be an integer value");
    elseif (sense >= 0)
      sense = 1;
    else
      sense = -1;
  ...
  if (nargin > 8)
    if (! isstruct (param))
      error ("highs: PARAM must be a structure");
    endif
  else
    param = struct ();
  endif
  ...
  [xopt, fmin, errnum, extra] = ...
    __highs__ (c, A, b, lb, ub, ctype, vartype, sense, param);
...
%! param.msglev = 0;
%! param.lpsolver = 1;
%! [xmin, fmin, errnum, extra] = highs (c, A, b, lb, ub, ctype, vartype, ...
%!   sense, param);
%! assert (fmin, c' * xmin);
%! for i = 1:3
%!   assert (A(i,:) * xmin <= b(i));
...
%!error <C .* finite values> highs (NaN, 2, 3)
%!error <A must be finite> highs (1, NaN, 3)
%!error <B must be finite> highs (1, 2, NaN)
%!error <LB must be a real-valued> highs (1, 2, 3, NaN)
%!error <UB must be a real-valued> highs (1, 2, 3, 4, NaN)
%!error <SENSE must be .* integer> highs (1, 2, 3, 4, 5, "F", "C", NaN)
